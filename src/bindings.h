...
enum class Device { None, Keyboard, Mouse, Gamepad };
...
inline const wchar_t* DeviceName(Device device) {
    switch (device) {
    case Device::None: return L"None";
...
inline std::wstring BindingLabel(const Binding& binding) {
    if (binding.device == Device::None) return L"Unassigned";
...
inline std::map<Action, Binding> Load(const std::filesystem::path& path) {
    ...
    while (std::getline(input, line)) {
        for (const auto& [action, unused] : Defaults()) {
            ...
            if (line.substr(deviceStart, 48).find(L"None") != std::wstring::npos) binding.device = Device::None;
            else if (line.substr(deviceStart, 48).find(L"Mouse") != std::wstring::npos) binding.device = Device::Mouse;
...
inline bool Pressed(const Binding& binding, const BYTE* keyboardState, uint32_t buttons, uint32_t dpad) {
    if (binding.device == Device::None) return false;
